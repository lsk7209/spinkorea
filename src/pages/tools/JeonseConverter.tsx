...
export default function JeonseConverter() {
  ...
  const j2m_monthly =
    j2m_effective > 0 && rate > 0 ? j2m_effective * rate : null;
      ...
      faqs={[
        {
          ...
          answer:
            "전세 보증금을 월세로 전환할 때 적용하는 이율입니다. 예를 들어 전환율 5.5% 기준으로 전세금 2억원은 월 91만원(2억 × 5.5% ÷ 12)에 해당합니다.",
        },
        {
          question: "법정 상한 전환율은 얼마인가요?",
          answer:
            "주택임대차보호법에 따라 전월세 전환율 상한은 '한국은행 기준금리 + 3.5%' 또는 '연 10%' 중 낮은 값입니다. 2024년 기준 약 7% 내외입니다.",
        },
        {
          question: "보증금 일부를 월세로 전환하는 방법은?",
          answer:
            "전세금에서 원하는 보증금을 뺀 금액(실질 전환 금액)에 전환율을 적용하면 됩니다. 예: 전세 2억, 보증금 3천만원 → 전환 금액 1.7억 × 전환율 ÷ 12 = 월세.",
        },
      ]}
      relatedTools={[
        {
          ...
          description: "할인율·비율 계산",
        },
        {
          name: "대출 계산기",
          path: "/tools/loan-calculator",
          description: "대출 이자·상환금 계산",
        },
        {
          name: "시급 계산기",
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="bg-white/5 border border-white/10 p-5 rounded-xl flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <input
              id="jeonse-converter-conv-rate"
              type="number"
              step="0.1"
              aria-label="전월세 전환율 (연 %)"
              value={convRate}
          ...
          <p className="text-xs text-gray-500 sm:ml-2">
            법정 상한 약 7% 이내 | 실 거래 시 협의 금액 직접 입력
          </p>
        </div>

        {/* 전세 → 월세 */}
        <div className="bg-white/5 border border-white/10 p-6 rounded-xl">
          <h3 className="text-lg font-bold text-neon-primary mb-5">
            전세 → 월세 환산
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              <label
                ...
                htmlFor="jeonse-converter-1"
              >
                전세 보증금 (원)
              </label>
              <input
                id="jeonse-converter-1"
                type="number"
                value={jeonseAmt}
              ...
              <p className="text-xs text-gray-500 mt-1">
                {parseFloat(jeonseAmt) > 0 ? toEok(parseFloat(jeonseAmt)) : ""}
              </p>
            </div>
            <div>
              <label
                className="block text-sm text-gray-400 mb-1"
                htmlFor="jeonse-converter-2"
              >
                월세 보증금 유지 (원, 없으면 0)
              </label>
              <input
                id="jeonse-converter-2"
                type="number"
                value={deposit}
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                {parseFloat(deposit) > 0 ? toEok(parseFloat(deposit)) : ""}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 p-6 rounded-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
              <label
                className="block text-sm text-gray-400 mb-1"
                htmlFor="jeonse-converter-3"
              >
                월세 (만원)
              </label>
              <input
                id="jeonse-converter-3"
                type="number"
                value={monthlyRent}
            ...
            <div>
              <label
                className="block text-sm text-gray-400 mb-1"
                htmlFor="jeonse-converter-4"
              >
                현재 보증금 (원)
              </label>
              <input
                id="jeonse-converter-4"
                type="number"
          ...
          <div className="bg-black/30 rounded-xl p-4 text-center">
            ...
            {m2j_equiv !== null && (
              <p className="text-xs text-gray-500 mt-1">{fmt(m2j_equiv)}원</p>
            )}
          </div>
        </div>
      </div>
