...
export default function DiffChecker() {
  ...
  return (
    <ToolLayout
      ...
      howToUse={[
        "왼쪽 입력창에 원본 텍스트를 입력하세요.",
        "오른쪽 입력창에 수정된 텍스트를 입력하세요.",
        "차이점이 색상으로 강조 표시됩니다.",
      ]}
      faqs={[
        {
          question: "어떤 색상이 무엇을 의미하나요?",
          answer:
            "초록색은 추가된 텍스트, 빨간색은 삭제된 텍스트를 나타냅니다. 변경되지 않은 부분은 기본 색상으로 표시됩니다.",
        },
        {
          question: "코드 비교도 가능한가요?",
          answer:
            "네, 일반 텍스트뿐만 아니라 프로그래밍 코드, JSON, 설정 파일 등 어떤 텍스트도 비교할 수 있습니다.",
        },
      ]}
      relatedTools={[
        {
          ...
          <div className="bg-black border border-white/10 rounded-xl p-6 min-h-[200px] font-mono whitespace-pre-wrap leading-relaxed text-gray-300">
            {diffs.map((part, index) => {
              const color = part.added
                ...
                : part.removed
                  ? "bg-red-500/30 text-red-200 decoration-red-400 line-through"
                  : "text-gray-400";
              return (
                <span
                  key={index}
                  className={`${color} px-0.5 rounded break-all`}
                >
                  {part.value}
                </span>
              );
