...
export default function CssShadowGenerator() {
  ...
  return (
    <ToolLayout
      ...
      howToUse={[
        "수평·수직 오프셋, 흐림도, 퍼짐 반경을 슬라이더로 조절하세요.",
        "그림자 색상을 선택하세요.",
        "미리보기로 결과를 확인하세요.",
        "생성된 CSS 코드를 복사하세요.",
      ]}
      faqs={[
        {
          question: "여러 개의 그림자를 동시에 적용할 수 있나요?",
          answer:
            "CSS box-shadow는 쉼표로 구분하여 여러 그림자를 적용할 수 있습니다. 생성된 코드를 수정하여 추가하세요.",
        },
        {
          question: "inset 그림자는 무엇인가요?",
          answer:
            "일반 그림자는 요소 바깥에 생기지만, inset을 추가하면 요소 안쪽에 그림자가 생겨 눌린 효과를 만들 수 있습니다.",
        },
      ]}
      relatedTools={[
        {
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-white/5 border border-white/10 p-6 rounded-xl space-y-6">
          <div className="flex justify-between items-center bg-white/5 p-3 rounded-lg">
            ...
            >
              <div
                className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${inset ? "translate-x-6" : "translate-x-0"}`}
              />
            </div>
          </div>
