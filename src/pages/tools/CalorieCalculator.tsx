...
export default function CalorieCalculator() {
  ...
  const goals = [
    ...
    },
    {
      ...
      cal: tdee,
      color: "text-cyan-400",
      badge: "bg-neon-primary/10 text-neon-primary",
    },
    {
      label: "근육 증가 (+0.25kg/주)",
      cal: tdee + 250,
      ...
      faqs={[
        {
          ...
          answer:
            "아무것도 하지 않고 24시간 누워 있어도 생명 유지를 위해 소비되는 최소 칼로리입니다. 호흡, 체온 유지, 장기 기능 등에 사용됩니다.",
        },
        {
          question: "TDEE와 BMR의 차이는?",
          answer:
            "TDEE(Total Daily Energy Expenditure)는 하루 총 소비 칼로리로, BMR에 활동량 계수를 곱한 값입니다. 실제 체중 관리에는 TDEE를 기준으로 섭취량을 조절합니다.",
        },
        {
          question: "얼마나 먹어야 살이 빠지나요?",
          answer:
            "체지방 1kg ≈ 7700kcal. 매일 500kcal를 TDEE보다 적게 먹으면 이론적으로 주 0.5kg 감량입니다. 단, 500kcal 이상 결핍은 장기간 지속 시 근육 손실을 유발할 수 있어 주의가 필요합니다.",
        },
      ]}
      relatedTools={[
        {
            ...
            {[
              ...
              <div key={label}>
                <label
                  ...
                  htmlFor={`calorie-calculator-${idx}`}
                >
                  {label}
                </label>
                <input
                  id={`calorie-calculator-${idx}`}
                  type="number"
                  value={value}
            ...
            <div className="space-y-2">
              {ACTIVITY_LEVELS.map((level, i) => (
                ...
                >
                  <div>
                    <span
                      className={`text-sm font-bold ${activityIdx === i ? "text-neon-primary" : "text-gray-300"}`}
                    >
                      {level.label}
                    </span>
          ...
          <div className="space-y-3">
            {goals.map((g) => (
              <div key={g.label} className="flex items-center justify-between">
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${g.badge}`}
                >
                  {g.label}
                </span>
                <span className={`text-base font-black ${g.color}`}>
                  {fmt(Math.max(g.cal, 1200))} kcal
                </span>
              </div>
            ))}
          </div>
          <p className="text-xs text-gray-500 mt-3">
            * 최소 1,200kcal(여성) / 1,500kcal(남성) 이하 섭취는 권장하지
            않습니다.
          </p>
        </div>
