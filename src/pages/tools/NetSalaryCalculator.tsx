...
// 근로소득 간이세액표를 단순화한 구간별 근사식(비부양가족 1인 기준)이며
// 국세청 공식 간이세액표(2026-03-01 개정본) 자체를 그대로 옮긴 것은 아닙니다.
// 정확한 세액은 홈택스 원천세 신고 메뉴의 간이세액표를 확인하세요.
...
export default function NetSalaryCalculator() {
  ...
  // 4대보험 (직원 부담, 2026년 기준)
  const nationalPension = Math.min(monthly * 0.0475, 302575); // 기준소득월액 상한 637만원 x 4.75%
  const healthInsurance = monthly * 0.03595; // 건강보험료율 7.19%의 절반(근로자 부담)
  const longTermCare = healthInsurance * 0.1314; // 장기요양보험료율(건강보험료 대비)
  ...
  return (
    <ToolLayout
      ...
      sources={[
        {
          name: "국민연금공단 — 보험료율 및 기준소득월액",
          url: "https://www.nps.or.kr",
        },
        {
          name: "국민건강보험공단 — 건강·장기요양보험료율",
          url: "https://www.nhis.or.kr",
        },
        {
          name: "국세청 홈택스 — 근로소득 간이세액표",
          url: "https://www.hometax.go.kr",
        },
      ]}
      reviewedAt="2026-09-12"
      disclaimer="4대보험 요율은 2026년 기준(국민연금 4.75%·상한 637만원, 건강보험 3.595%, 장기요양 건강보험료의 13.14%, 고용보험 0.9%)이며, 소득세는 국세청 공식 간이세액표를 단순화한 근사값입니다. 실제 공제액은 홈택스 원천세 메뉴에서 다시 확인하세요."
      howToUse={[
        "세전 연봉을 입력하세요.",
      ...
      tips={[
        "소득세는 간이세액표를 단순화한 근사값이며 실제와 다를 수 있습니다. 정확한 금액은 홈택스에서 확인하세요.",
        "국민연금은 기준소득월액 637만원 초과분에는 보험료가 추가되지 않습니다(2026년 기준 월 상한 302,575원).",
      ...
      faqs={[
        {
          ...
          answer:
            "국민연금·건강보험·고용보험·산재보험입니다. 이 중 산재보험은 전액 사업주 부담이라 직원 급여 공제에서 제외됩니다.",
        },
        {
          question: "연봉 협상 시 식대, 교통비 포함 여부가 중요한가요?",
          answer:
            "식대(월 20만원 이하)와 교통비 일부는 비과세 항목입니다. 연봉에 이 항목이 포함된 경우 과세 구간이 낮아져 세금이 줄고 실수령이 늘어납니다.",
        },
        {
          question: "퇴직금은 연봉에 포함되나요?",
          answer:
            "연봉 계약 방식에 따라 다릅니다. '퇴직금 별도'인 경우 여기 입력한 연봉으로 실수령을 계산하면 됩니다. '퇴직금 포함'이라면 연봉의 약 1/13을 빼고 입력하세요.",
        },
      ]}
      relatedTools={[
        {
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="bg-white/5 border border-white/10 p-6 rounded-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1" htmlFor="net-salary-calculator-1">
                세전 연봉 (원)
              </label>
              <input id="net-salary-calculator-1"
                type="number"
                value={annualSalary}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "월 공제 합계",
              value: `${fmt(totalDeduction)}원`,
              color: "text-red-400",
            },
            {
              label: "실수령 월급",
              value: `${fmt(netMonthly)}원`,
              color: "text-cyan-400",
            },
            {
              label: "실수령 연봉",
              value: `${fmt(netAnnualAmt)}원`,
              color: "text-green-400",
            },
        ...
        <div className="bg-white/5 border border-white/10 p-6 rounded-xl">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {[2400, 3000, 3600, 4000, 5000, 6000, 8000, 10000].map(
                  (amt) => {
                    const m = (amt * 10000) / 12;
                    const np = Math.min(m * 0.0475, 302575);
                    const hi = m * 0.03595;
                    const lt = hi * 0.1314;
                    const ei = m * 0.009;
                    ...
                    const isCurrentRange =
                      Math.abs(annual - amt * 10000) < 500000;
                    return (
                      <tr
                        key={amt}
                        className={`border-b border-white/5 hover:bg-white/5 ${isCurrentRange ? "bg-neon-primary/10" : ""}`}
                      >
                        <td
                          className={`py-2 pr-4 ${isCurrentRange ? "text-neon-primary font-bold" : "text-gray-400"}`}
                        >
                          {(amt / 100).toFixed(0)}백만원
                        </td>
                        <td className="py-2 pr-4 text-white">{fmt(m)}원</td>
                        <td
                          className={`py-2 font-medium ${isCurrentRange ? "text-neon-primary" : "text-cyan-400"}`}
                        >
                          {fmt(net)}원
                        </td>
                      </tr>
          ...
          <p className="text-xs text-gray-500 mt-2">
            * 간이 추정치이며 비과세 수당, 부양가족 공제 등에 따라 실제와
            다릅니다.
          </p>
        </div>
      </div>
