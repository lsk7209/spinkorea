...
export default function Base64Encoder() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setError(
        mode === "encode" ? "인코딩 실패" : "유효하지 않은 Base64 형식입니다.",
      );
    }
  }, [input, mode]);
  ...
  return (
    <ToolLayout
      ...
      howToUse={[
        "변환할 텍스트를 입력하세요.",
        "'인코딩' 버튼으로 텍스트→Base64, '디코딩' 버튼으로 Base64→텍스트로 변환하세요.",
        "결과를 복사하세요.",
      ]}
      faqs={[
        {
          question: "Base64 인코딩은 무엇인가요?",
          answer:
            "이진 데이터를 ASCII 문자로 변환하는 방식입니다. 이메일 첨부파일, 이미지 데이터 URL, API 인증(Basic Auth) 등에 사용됩니다.",
        },
        {
          question: "Base64는 암호화인가요?",
          answer:
            "아닙니다. Base64는 암호화가 아닌 인코딩 방식입니다. 누구나 디코딩할 수 있으므로 민감한 정보 보호에는 적합하지 않습니다.",
        },
      ]}
      relatedTools={[
        {
        ...
        },
        {
          ...
          description: "JSON 정렬·압축",
        },
        {
          name: "비밀번호 생성기",
          path: "/tools/random-password",
          description: "강력한 보안 암호",
        },
      ]}
    >
      <div className="flex flex-col gap-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-8 items-start relative">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-gray-400 text-sm font-medium ml-1 flex justify-between" htmlFor="base64-encoder-1">
              <span>입력 (Input)</span>
              <button
                onClick={() => setInput("")}
            ...
            <textarea id="base64-encoder-1"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={
                mode === "encode"
                  ? "변환할 텍스트를 입력하세요 (예: 안녕하세요)"
                  : "디코딩할 Base64 코드를 입력하세요 (예: 5y+T5Lm45ZXZ)"
              }
          ...
          <div className="flex flex-col gap-2 relative">
            <label className="text-gray-400 text-sm font-medium ml-1" htmlFor="base64-encoder-2">
              결과 (Result)
            </label>
            <textarea id="base64-encoder-2"
              value={error || output}
              readOnly
              placeholder="변환 결과가 여기에 표시됩니다."
              className={`w-full h-64 bg-black/40 border rounded-xl p-4 text-sm font-mono leading-relaxed focus:outline-none resize-none break-all ${
                error
                  ? "border-red-500/50 text-red-400"
                  : "border-white/10 text-neon-secondary"
              }`}
            />
        ...
        <div className="bg-white/5 border border-white/10 p-5 rounded-xl mt-4">
          ...
          <p className="text-sm text-gray-400 leading-relaxed">
            바이너리 데이터(이미지, 파일 등)를 텍스트(ASCII) 형태로 안전하게
            전송하기 위해 사용하는 인코딩 방식입니다. 이메일 첨부파일이나 HTML
            내 이미지 삽입(Data URI) 등에 자주 사용됩니다.
            <strong>SpinFlow</strong>는 한글 깨짐 방지를 위해 UTF-8 처리를
            기본으로 지원합니다.
          </p>
        </div>
      </div>
