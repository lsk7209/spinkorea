...
function withObjectPhrase(value: string): string {
  return value.endsWith("확인") || value.endsWith("점검") ? `${value} 항목을` : withEulReul(value);
}

function withEunNeun(value: string): string {
...
function withIga(value: string): string {
...
function toneVariant(article: GeneratedArticlePlan): number {
  const number = Number(article.id.replace(/\D/g, ""));
...
function buildFaq(article: GeneratedArticlePlan) {
  return [
    {
      question: `${withObjectPhrase(article.mainKeyword)} 처음 적용할 때 가장 먼저 볼 것은 무엇인가요?`,
      answer: `${withWaGwa(keywordAt(article, 0))} ${withObjectPhrase(keywordAt(article, 1))} 먼저 나누면 ${withEulReul(article.readerProblem)} 줄일 수 있습니다. 선택지를 늘리기보다 기준과 예외를 먼저 정하는 편이 안정적입니다.`,
    },
    {
      question: `${article.practicalExample}에는 어떤 기준이 실용적인가요?`,
      answer: `${withObjectPhrase(keywordAt(article, 2))} 기준으로 시작하고, 결과 공유가 필요하다면 ${keywordAt(article, 3)}까지 남기는 방식이 좋습니다. 그래야 다음에도 같은 상황을 설명하기 쉽습니다.`,
    },
    {
    ...
    {
      question: `${withObjectPhrase(article.mainKeyword)} 글이나 도구 페이지에 연결할 때 주의할 점은 무엇인가요?`,
...
function TypeSpecificSection({ article }: { article: GeneratedArticlePlan }) {
      ...
      <>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {[
            ...
            ["공유 후", `${withObjectPhrase(keywordAt(article, 3))} 확인해 다음 실행에 같은 기준을 씁니다.`],
          ].map(([phase, text]) => (
  ...
  if (article.contentType === "Comparison") {
    return (
      <>
        <h2>{withObjectPhrase(article.mainKeyword)} 감으로 고를 때와 기준으로 고를 때 차이</h2>
        <div className="overflow-x-auto">
          <table>
            ...
            <tbody>
              ...
              <tr>
                <td>기준 선택</td>
                <td>{withObjectPhrase(keywordAt(article, 0))} 설명하기 쉽습니다</td>
    ...
    return (
      <>
        ...
        <p>
          ...
          {" "}
          {keywordAt(article, 0)}, {keywordAt(article, 1)}, {withObjectPhrase(keywordAt(article, 2))} 질문 단위로 나누면
  ...
  if (article.contentType === "Explainer") {
    return (
      <>
        <h2>{withObjectPhrase(article.mainKeyword)} 쉽게 이해하는 기준</h2>
        <p>
          {withEunNeun(article.mainKeyword)} 결과만 보면 단순해 보이지만 실제로는 {keywordAt(article, 0)},{" "}
          {keywordAt(article, 1)}, {withEunNeun(keywordAt(article, 2))} 함께 움직입니다. 이 세 가지를 분리하면
  ...
  return (
    <>
      ...
      <p>
        먼저 {withObjectPhrase(keywordAt(article, 0))} 정하고, 그다음 {withObjectPhrase(keywordAt(article, 1))} 확인합니다. 마지막으로{" "}
        {withWaGwa(keywordAt(article, 2))} {withObjectPhrase(keywordAt(article, 3))} 남기면 다음번에도 같은 판단을 반복하지 않아도 됩니다.
      </p>
    </>
  );
}
...
function ContextSection({ article }: { article: GeneratedArticlePlan }) {
  ...
  if (variant === 1) {
    return (
      <>
        <h2>{withObjectPhrase(article.mainKeyword)} 실제 상황에 맞추는 방법</h2>
        <p>
          {article.practicalExample}에는 정답 하나보다 납득 가능한 절차가 더 중요합니다. 먼저{" "}
          {withObjectPhrase(keywordAt(article, 0))} 확인하고, 이해관계자가 헷갈릴 수 있는{" "}
          {withObjectPhrase(keywordAt(article, 1))} 짧게 적어두면 결과 설명이 쉬워집니다.
        </p>
      </>
    );
  }
  ...
  if (variant === 2) {
    return (
      <>
        <h2>{withObjectPhrase(article.mainKeyword)} 문서로 남길 때 필요한 것</h2>
        <p>
          실행 후에는 결과만 저장하지 말고 왜 그 방식으로 정했는지 함께 남기는 편이 좋습니다. {withWaGwa(keywordAt(article, 2))}{" "}
          {withObjectPhrase(keywordAt(article, 3))} 같이 적으면 나중에 같은 조건을 다시 확인할 때 설명 비용이 줄어듭니다.
        </p>
      </>
    );
  }
    ...
    return (
      <>
        ...
        <p>
          ...
          덜어내고 {withWaGwa(keywordAt(article, 0))} {keywordAt(article, 2)}만 먼저 남기면 실행 속도가 빨라집니다.
        </p>
      </>
    );
  }
  ...
  if (variant === 4) {
    return (
      <>
        <h2>{withObjectPhrase(article.mainKeyword)} 반복 운영할 때의 기준</h2>
        <p>
          한 번 쓰고 끝나는 기준과 매번 다시 쓰는 기준은 다르게 설계해야 합니다. 반복 운영에서는 {withObjectPhrase(keywordAt(article, 1))}
  ...
  return (
    <>
      <h2>{withObjectPhrase(article.mainKeyword)} 빠르게 점검하는 관점</h2>
      <p>
        가장 먼저 볼 것은 결과의 화려함이 아니라 독자가 바로 따라 할 수 있는지입니다. {withWaGwa(keywordAt(article, 0))}{" "}
        {withIga(keywordAt(article, 1))} 본문 안에서 자연스럽게 이어지면 검색 의도와 실행 흐름이 같이 살아납니다.
      </p>
    </>
  );
}
...
function GeneratedArticle({ article }: { article: GeneratedArticlePlan }) {
  ...
  const tone = toneVariant(article);
  const opening = [
    `${withEunNeun(article.mainKeyword)} 막연한 감으로 처리하면 시간이 늘어지는 주제입니다.`,
    `${withObjectPhrase(article.mainKeyword)} 다룰 때는 빠른 결론보다 다시 설명할 수 있는 절차가 중요합니다.`,
    `${article.mainKeyword} 상황은 작은 기준 차이만으로도 결과 해석이 달라질 수 있습니다.`,
    `${withEunNeun(article.mainKeyword)} 한 번 정리해 두면 다음 선택과 공유 과정이 훨씬 단순해집니다.`,
  ][tone];
  const summaryLast = [
    "발행, 공유, 업무 적용 전에는 체크리스트로 한 번 더 검토하는 편이 안전합니다.",
    "결과보다 절차를 함께 남기면 다음번 판단 비용이 줄어듭니다.",
    "내부 링크와 공식 출처를 같이 두면 독자가 읽은 뒤 바로 실행할 수 있습니다.",
    "기준을 짧게 공개하면 참여자나 독자가 결과를 받아들이기 쉬워집니다.",
  ][tone];
  const checklist = [
    `${withObjectPhrase(article.mainKeyword)} 쓰는 목적을 한 문장으로 적기`,
  ...
  return (
    <div className="space-y-7">
      <p>
        {opening} 이 글은 {article.readerProblem} 상황에서 바로 적용할 수 있도록 절차, 확인 항목,
        기록 방식을 함께 정리했습니다.
      </p>
      ...
      <div className="rounded-xl border border-cyan-100 bg-cyan-50 p-5 text-slate-800">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5">
          <li>{withEunNeun(article.mainKeyword)} 목적과 제외 조건을 먼저 정해야 흔들리지 않습니다.</li>
          <li>{article.expandedKeywords.slice(0, 2).join(", ")}를 나눠 보면 실행 순서가 분명해집니다.</li>
          <li>{summaryLast}</li>
        </ul>
      </div>

      <h2>{withEunNeun(article.mainKeyword)} 언제 필요할까요?</h2>
      <p>
        가장 흔한 장면은 {article.practicalExample}입니다. 이때 사람들은 빠른 결론을 원하지만, 기준이 없으면
        같은 결정을 반복해서 다시 논의하게 됩니다. {withObjectPhrase(article.mainKeyword)} 별도 기준으로 분리하면 선택지,
        제약, 결과를 나눠 볼 수 있어 실수가 줄어듭니다.
      </p>
      <p>
        특히 {withWaGwa(keywordAt(article, 0))} {withIga(keywordAt(article, 1))} 함께 걸린 상황에서는 단순 추천보다 재사용 가능한
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{withObjectPhrase(keywordAt(article, 2))} 기준으로 제외 조건을 둡니다</td>
            </tr>
            <tr>
              <td>검증</td>
              <td>결과가 실제로 쓸 만한가?</td>
      ...
      <p>
        아래 도구는 {withWaGwa(article.mainKeyword)} 직접 연결되는 흐름입니다. 글을 읽은 뒤 바로 실행할 수 있도록 내부
        링크를 함께 정리했습니다.
      </p>
      <ul>
